...
-- by @wim66
-- v 0.7.1 May 22, 2025(sticky top city restored)
...
-- Function: draw_image
-- Loads and draws a PNG image at the specified position and height
...
function conky_draw_text()
          if conky_window == nil then
                    return
          end -- Exit if Conky window is not available
          ...
          local y1 = settings.y + settings.line_height
          ...
          local bg_y = y1 - settings.flag_y_offset - settings.label_padding_y
          ...
          draw_image(cr, priority_city.flag, flag_x, y1 - settings.flag_y_offset, settings.flag_height)

          -- Text
          set_rgba(cr, priority_city.color, priority_city.alpha)
          cairo_move_to(cr, settings.x, y1)
          ...
          for i = 1, visible_lines do
                    ...
                    local y2 = settings.y + (i + 1) * settings.line_height -- +1 to start below Amsterdam
                    ...
                    local text_width = extents.width
                    local bg_x = settings.x + settings.flag_x_offset - settings.label_padding_x
                    local bg_y = y2 - settings.flag_y_offset - settings.label_padding_y
                    local bg_w = settings.flag_height + -settings.flag_x_offset + text_width - 15 + settings.label_padding_x * 2
                    local bg_h = settings.flag_height + settings.label_padding_y * 2
                    ...
                    -- Flag
                    local flag_x = settings.x + settings.flag_x_offset
                    draw_image(cr, city.flag, flag_x, y2 - settings.flag_y_offset, settings.flag_height)

                    -- Text
                    set_rgba(cr, city.color, city.alpha)
                    cairo_move_to(cr, settings.x, y2)
                    cairo_show_text(cr, label_text)
          end
